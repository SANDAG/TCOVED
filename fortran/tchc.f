...
c     *tdzmrt(mxtdz),tdzrt(mxtdz,mxzrt),
c     *tdzrtnode(mxtdz,mxzrt),tdzrtid(mxtdz,mxzrt),taztdz(mxzn)
      real adtvols(29),adthst(mxradt)
     ...
     *jrmile(3,10),jlmile(3,10),jcmile(3,10),jrhour(3,10)
      real*8 aoc, mlcaprate, fwycaprate
      ...
      character*80 aocstr, mlcapstr, fwycapstr
      data angle/0,-90,90/
...
c
      ...
      call getProperty(9998,'aoc',aocstr)
      call strToReal(aocstr,aoc)
      if(aoc.le.0) go to 9005

	  call getProperty(9998,'mlcaprate',mlcapstr)
      call strToReal(mlcapstr,mlcaprate)
      if(mlcaprate.le.0) go to 9006
	  
	  call getProperty(9998,'fwycaprate',fwycapstr)
      call strToReal(fwycapstr,fwycaprate)
      if(fwycaprate.le.0) go to 9007
c
...
c      end do
   89 write(0,11002)
c      fnami='../data/taz.tdz'
c      call opendata(lunin)
c	if(istat.ne.0) go to 9000
c   95 read(lunin,10000,iostat=istat) idata,adata
c      if(istat.ne.0) go to 96
c      call sd_getdat(idata,adata,icol,izone,itdz,i3,lunerr)
c      if(izone.lt.1.or.izone.gt.mxzn) go to 9001
c      taztdz(izone)=itdz
c      go to 95
c   96 close(lunin)
c      fnamo='../covs/tdzs/pat.adf'
c      open(unit=patlun,convert='big_endian',file=fnamo,access=
c     *'direct',form='unformatted',recl=12,status='old',err=9000)
c       patrec=2 
c   98 call rzonep	
c      if(patio.ne.0) go to 99
c      xtdz(patzon)=float(patx)
c      ytdz(patzon)=float(paty)	
c      patrec=patrec+1
c      go to 98
c   99 close(patlun)
      call ohca
...
c      xnode(nattpn)=float(natx)
c      ynode(nattpn)=float(naty)
      nattmp=0
...
c      xn=xnode(jn)
c      yn=ynode(jn)
c      xdist=sqrt((xz-xn)**2+(yz-yn)**2) !distance from tdz centroid to node
c      mzrt=tdzmrt(itdz)
c      mnrt=nodemrt(jn)
c       if(mzrt.gt.0) then !if tdz-route connenctions already exist then find out if this is a new one
c        do irt=1,mnrt !loop through routes at this node
c          irtno=nodert(jn,irt)
c          new=.true.

c           do jrt=1,mzrt !loop through existing connections
c            jrtno=tdzrt(itdz,jrt)
c            zdist=tdzrtdist(itdz,jrt)
c             if(irtno.eq.jrtno) then !if new route matches old route then this isn't a new connection
c              new=.false. 
c               if(xdist.lt.zdist) then !if it is an old connection this may be a shorter distance
c                tdzrtdist(itdz,jrt)=xdist
c                tdzrtnode(itdz,jrt)=jn
c               end if !end of distance check
c             end if !end of new connection check
c            end do !end of extisting tdz-route connections loop

c          if(new) then !if this is a new route connection then add to matrix 
c          mzrt=mzrt+1
c            if(mzrt.gt.mxzrt) then
c             write(lunerr,19582)
c             errf=.true.
c             go to 9999
c            endif
c           mmzrt=max(mmzrt,mzrt)
c           tdzmrt(itdz)=mzrt
c           tdzrt(itdz,mzrt)=irtno
c           tdzrtdist(itdz,mzrt)=xdist
c           tdzrtnode(itdz,mzrt)=jn
c           tdzrtid(itdz,mzrt)=aatid !re-use id numbers for tdz connections
c          endif !end of new check
c        end do !end of loop through node routes
c       else !this is the first connection for tdz
...
c       endif !end of number of connections check
c
...
c          errf=.true.
  ...
  601 if(iyear.ge.2015) then
     ...
     *fwycap=float(aatplc(1))
	  ...
	  !Modified by Ziying Ouyang, March 5, 2015
	  ! Freeway capacity override
	  if(aathov(1).eq.1)xcap = xcap*fwycaprate
      if(aathov(1).eq.2.or.aathov(1).eq.3) then
       !changed from 1600 to 2000. Consistent with GP Lanes
       xcap=float(aatln(ipk,idir,1))*2000.0      
      endif
	!Modified by Ziying Ouyang, Dec 10,2014
	! ML capacity override
	if(aathov(1).eq.3.and.aatln(ipk,idir,1).ne.9)xcap = xcap*mlcaprate
	if(aatprj(1).eq.613.or.aatprj(1).eq.614)xcap = xcap*mlcaprate
  ...
  605 i=index(aatnm,'ACCESS')
	  ...
	  if (aathov(1).gt.1) xcap = xcap*mlcaprate
	  if (aathov(1).eq.1) xcap = xcap*fwycaprate !modified by ZOU, March 5, 2015
        go to 608
       endif
...
c      print *, mmzrt
c      do itdz=1,mxtdz !loop through tdzs
c       mzrt=tdzmrt(itdz)
c        if(mzrt.lt.1) then !make sure at lest one connection was found for tdz
c         write(lunerr,19891) itdz
c         errf=.true.
c        else
  
c         do irt=1,mzrt !loop through route connections for this tdz
c          in=tdzrtnode(itdz,irt)
c          id=tdzrtid(itdz,irt)
c           if(.not.nused(in)) then !check to see if tdz-node connection was already made for previous route
C	Added by Ziying Ouyang, Oct 29, 2009 for error checking	
c	      if (xtdz(itdz).gt.0.and.ytdz(itdz).gt.0) then	        
c               write(lunxy,20401) 
c     *         id,xtdz(itdz),ytdz(itdz),xnode(in),ynode(in)
c               nused(in)=.true.
c	      else
c               write(lunerr,19915) id,itdz !invalid x y coordinates
c	     endif
c	   endif
c         end do !end of route connection loop

c	 do in=1,mxhcn !reset node used to zero
c          nused(in)=.false.
c         end do
c       end if !end of check fo tdz connections
c      end do ! end of tdz loop
c      write(lunxy,20409) 
c
...
c     if toll facilities or hov3+ facilities found then write out flag files
 ...
 9001 write(lunerr,19001) fnamo
      errf=.true.
      go to 9999
 9005 write(lunerr,19005) aoc
 9006 write(lunerr,19006) mlcaprate
 9007 write(lunerr,19007) fwycaprate
...
20000 format(a1,1x,a8,t57,'highway coverage arc data',
     ...
     *'      capacity')
20001 format(/,t36,'route number: ',i5,' name: ',a20,1x,a7,1x,a12,/,
     *13x,a20)
...
19005 format(' auto operating cost is incorrect: ',a10)
19006 format(' manage lane cap increase rate is incorrect: ',a10)
19007 format(' freeway lane cap increase rate is incorrect: ',a10)
...
c 19891 format(' missing zone connectors for tdz',i6)
...
c 19915 format(' hwycovtdz zone connector 
c     * invalid x, y coordinate for link id ',i8,'for tdz ',i8)
